...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[CROMEX CM1-ONLY] Enviando requisição para API Python: ${pricingApiUrl}/run-cm1`);

    const pythonRes = await fetch(`${pricingApiUrl}/run-cm1`, {
      method: 'POST',
      headers: {
    ...
    if (!pythonRes.ok) {
      ...
      console.error(`[CROMEX CM1-ONLY] Erro da API Python: ${errText}`);
      return NextResponse.json({ error: 'Falha no processamento do CM1' }, { status: 500 });
    }

    const data = await pythonRes.json();
    return NextResponse.json(data);
  } catch (error: any) {
    console.error('Erro ao disparar pipeline de CM1:', error);
    return NextResponse.json({ error: 'Erro ao disparar cálculo de CM1' }, { status: 500 });
  }
}
